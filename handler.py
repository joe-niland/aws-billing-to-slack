...
def report_cost(
    ...
    list_of_dates = [(week_ago + datetime.timedelta(days=x)).strftime("%Y-%m-%d") for x in range(n_days)]
    ...
    query = {
        "TimePeriod": {"Start": week_ago.strftime("%Y-%m-%d"), "End": today.strftime("%Y-%m-%d")},
        "Granularity": "DAILY",
    ...
    buffer += (
        f"{'Other':{longest_name_len}} ${other_costs[-1]:8,.2f} {delta(other_costs):4.0f}% {sparkline(other_costs):7}\n"
    )
    ...
    buffer += (
        f"{'Total':{longest_name_len}} ${total_costs[-1]:8,.2f} {delta(total_costs):4.0f}% {sparkline(total_costs):7}\n"
    )
    ...
    if credits_expire_date:
        ...
        summary = (
            f"{emoji} Yesterday's cost for {account_name_label} ${total_costs[-1]:,.2f} "
...
def publish_slack(hook_url, summary, buffer):
    resp = requests.post(hook_url, json={"text": summary + "\n\n```\n" + buffer + "\n```"})
...
if __name__ == "__main__":
    ...
    summary, buffer, cost_dict = report_cost(
        None, None, "UnblendedCost", example_result, yesterday="2021-08-23", new_method=False
    )
    assert "{0:.2f}".format(cost_dict.get("total", 0.0)) == "286.37", f'{cost_dict.get("total"):,.2f} != 286.37'
    summary, buffer, cost_dict = report_cost(
        None, None, "UnblendedCost", example_result2, yesterday="2021-08-29", new_method=False
    )
    assert "{0:.2f}".format(cost_dict.get("total", 0.0)) == "21.45", f'{cost_dict.get("total"):,.2f} != 21.45'
